...
import app.services.base


class StructureTestSuite(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.MODULE = app.services.base

    def test_class_exists_basecrmservice(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"QmFzZUNSTVNlcnZpY2U=").decode(),
            classes,
            msg=f"The class "
            f"`{base64.b64decode(b'QmFzZUNSTVNlcnZpY2U=').decode()}` "
            f"is not found, but it was marked as required.",
        )

    def test_class_function_exists_basecrmservice___init__(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"QmFzZUNSTVNlcnZpY2U=").decode(),
            classes,
        ...
        functions = _get_class_function_names(
            self.MODULE, base64.b64decode(b"QmFzZUNSTVNlcnZpY2U=").decode()
        )
        self.assertIn(
            base64.b64decode(b"QmFzZUNSTVNlcnZpY2UuX19pbml0X18=").decode(),
            functions,
            msg=f"The function "
            f"`{base64.b64decode(b'QmFzZUNSTVNlcnZpY2UuX19pbml0X18=').decode()}` "
            f"is not found in class "
            f"`{base64.b64decode(b'QmFzZUNSTVNlcnZpY2U=').decode()}`, "
            f"but it was marked as required.",
        )

    def test_class_function_signature_match_basecrmservice___init__(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"QmFzZUNSTVNlcnZpY2U=").decode(),
            classes,
        ...
        self.assertEqual(
            base64.b64decode(b"Y2xpZW50X2ZhY3Rvcnk=").decode(),
            args[1],
